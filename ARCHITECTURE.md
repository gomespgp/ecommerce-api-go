...
| **Dependency Injection Container** | **Self-Contained Module Registration** | We don't use heavy frameworks. Each feature package exposes a single registration function (e.g., `RegisterItemRoutes`) that handles its own internal layer wiring (`Repository` ➔ `Service` ➔ `Handler`) from the DB Pool down to the router. |
...
```text
 ...
 [ Router (cmd/api/main.go) ]
      │
      │ 2. Delegates to item.RegisterItemRoutes(dbPool, r)
      ▼
 [ Route & Wiring Layer (internal/item/route.go) ]
      │
      │ 3. Instantiates Repo -> Service -> Handler & maps HTTP verbs to handler methods
      ▼
 [ Presentation Layer: Handler (internal/item/handler.go) ]
      │
      │ 4. Decodes JSON into `createItemRequest` struct
      │ 5. Extracts parameters & calls h.service.Create()
      ▼
 [ Business Logic Layer: Service (internal/item/service.go) ]
      │
      │ 6. Validates parameters (e.g. Price > 0)
      │ 7. Converts float64 to pgtype.Numeric (Postgres compatible)
      │ 8. Assembles `sql.CreateItemParams` and calls s.repo.Create()
      ▼
 [ Data Access Layer: Repository (internal/item/repository.go) ]
      │
      │ 9. Executes `r.queries.CreateItem(ctx, arg)`
      ▼
 [ Autogenerated Database Client (db/sqlc/items.sql.go) ]
      │
      │ 10. Queries Postgres using safe parameterized SQL inserts
      ▼
 [ PostgreSQL Database ]
```

### Step 1: Receiving, Wiring, & Routing (`internal/item/route.go`)
An HTTP request is sent by the client. The `main.go` file delegates setup to `item.RegisterItemRoutes(dbPool, r)`, which performs internal dependency wiring and registers all endpoint paths to the handler:

```go
func RegisterItemRoutes(dbPool *pgxpool.Pool, r chi.Router) {
    // 1. Internal Layer Wiring
    repo := NewRepository(dbPool)
    service := NewService(repo)
    handler := NewHandler(service)

    // 2. Route Definition & Mapping
    r.Route("/items", func(r chi.Router) {
        r.Post("/", handler.Create) // Maps POST /items to handler.Create method
        // ... other item routes
    })
}
```
...
3. **Module Registration:** It configures the **Chi** router, applies standard middleware (logging, timeout, panic recovery), and registers domain modules concisely without cluttering `main.go` with internal layer dependencies:
   ```go
...
#### 1. `route.go` (Wiring & Route Setup)
This file acts as the single entry point for the module. It instantiates the repository, service, and handler using the provided `*pgxpool.Pool`, then binds all endpoints (`/items`, `/items/{id}`, etc.) to the Chi router.
...
#### 4. `handler.go` (Transport Layer)
This is the HTTP adapter.
* It implements the endpoints registered in `route.go`.
...
#### Step 6: Map Route to Handler in router (`internal/item/route.go`)
Register the endpoint inside the `/items` route group inside `route.go`:
```go
r.Route("/items", func(r chi.Router) {
    // ... existing routes ...
...
Create a new folder `internal/order` containing four standard clean architecture files:

##### 1. `internal/order/route.go`
```go
package order
...
func RegisterOrderRoutes(dbPool *pgxpool.Pool, r chi.Router) {
    repo := NewRepository(dbPool)
    service := NewService(repo)
    handler := NewHandler(service)
...
##### 2. `internal/order/repository.go`
```go
package order
...
import (
    ...
    "errors"
    "fmt"
    "[github.com/jackc/pgx/v5/pgtype](https://github.com/jackc/pgx/v5/pgtype)"
)
...
#### Step 5: Register routes in the Server Orchestrator (`cmd/api/main.go`)
1. Import the new order domain:
   ...
   ```
2. Call the domain router registration function:
   ```go
   // --- Register domain modules ---
   item.RegisterItemRoutes(dbPool, r)
   order.RegisterOrderRoutes(dbPool, r)
   ```

With these steps, your architecture documentation now accurately reflects the self-contained module pattern!